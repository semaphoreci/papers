...
If so, it’s all right—you’re not alone—but it doesn’t have to be that way. Many methods can help us make safe deployments without downtime or the need for maintenance windows. One of these methods is called *blue-green* (or *blue/green*), and that’s what we’ll learn about today.

### What is Blue-Green Deployment?

Blue-green deployment is a software engineering technique that reduces risk and minimizes downtime. It uses two production environments, known as Blue and Green, to provide accurate testing, no-outage upgrades, and instant rollbacks.
...
Now I know what you’re thinking. Isn’t testing in production a big no-no? Normally, yes. But you see, the key point here is that they weren’t *overwriting* the old site. Instead, they were running the new one *side-by-side*, in the same physical box. Users were unaware of the ongoing deployment. The old site continued working as usual while Dan and Jez worked on the release.
...
![](./public/bluegreen-general/folders.png)

At this point, they had two environments on the same machine: one for the old version and another for the new. Initially, they wanted to call them by letters: *environment A*, *environment B*, and so on. But someone pointed out that people would tend to believe that A is somehow better than B (maybe it sounded too much like “plan B”). They finally settled on using colors instead, which didn’t have a natural order. Thus, they planned names like *blue*, *green*, or *orange* (they avoided *red* because it implied danger). In the end, it turned out they only needed two environments. And so the term blue-green was coined.
...
![](./public/bluegreen-general/bg2.png)
...
![](./public/bluegreen-general/bg3.png)
...
## The Pros and the Cons of Blue-Green Deployments
...
At this point, you might be thinking that blue-green must have a catch. Otherwise, everyone that could would be using it. So, let’s examine the drawbacks:
  ...
  - **Time**: setting up the blue-green deployment process takes time and effort. The process is complex and has a great deal of responsibility. We may need many iterations to get it working right.
...
We have learned what blue-green deployments are, how they came to be, and the problems they solve. I hope this post helps you decide if blue-green is the right deployment method for you.

Learn about more ways of deploying software with these posts:

  - We’re working on a blue-green on Kubernetes step-by-step guide. Don’t miss it! Subscribe to our [newsletter](https://semaphoreci.com/newsletter), and we’ll let you know as soon as it’s published.
